...
export default class ReplenishmentWO extends LightningElement {
    recordId;
    @track searchKey = '';
    @track fieldTechId = '';
    @track selectedValue = '';
    ...
    getStateParameters(currentPageReference) {
        if (currentPageReference) {
            ...
            console.log('recordId  currentPageReference 2-> ' + this.recordId);
        }
    }
    ...
    handleClickMore2(event){
        this.limitNumber2 = this.limitNumber2 + 100;
        this.retrieveMinQtyProds();
    }
                            ...
                            for(var p = 0; p < prodItems.length; p++){
                                ...
                                if(prodItems[p].Location.LocationType == 'Van'){
                                    ...
                                    if(prodItems[p].Minimum_Quantity__c != 0 && prodItems[p].Minimum_Quantity__c > prodItems[p].QuantityOnHand) {
                                        ...
                                        console.log('Quantity To Order-> ' + this.allCubProductList[prod].Quantity__c);
                                    }
                                } else {
        ...
        .then((result) => {
            if (result.length === 0) {
                ...
                setTimeout(() => {
                    ...
                    var totalProds = Object.keys(result)[0];
                    console.log('totalProds-> ' + totalProds);
                    console.log('All CUB Products Retrieved-> ' + JSON.stringify(this.minQtyProdList));
                    if(totalProds == this.minQtyProdList.length) this.stillProdsExists2 = false;
                    console.log('Min Qty Products Retrieved-> ' + JSON.stringify(result));
                        ...
                        for(var p = 0; p < prodItems.length; p++){
                            ...
                            if(prodItems[p].Location.LocationType == 'Van'){
                                ...
                                if(prodItems[p].Minimum_Quantity__c != 0 && prodItems[p].Minimum_Quantity__c > prodItems[p].QuantityOnHand) {
                                    ...
                                    console.log('Quantity To Order-> ' + this.minQtyProdList[prod].Quantity__c);
                                }
                            } else {
                                prodItems[p].isVanLocation = false;
                            }
                    ...
                    if(this.addedProductList.length != 0){
                        for (var p = 0; p < this.addedProductList.length; p++) {
                            for (var a = 0; a < this.minQtyProdList.length; a++) {
                                if (this.minQtyProdList[a].Id == this.addedProductList[p].Id) {
                                    this.minQtyProdList[a].isSelected = true;
                                    this.minQtyProdList[a].Quantity__c = this.addedProductList[p].Quantity__c;
                                }
                            }
                        }
                    }

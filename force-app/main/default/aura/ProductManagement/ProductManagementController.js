({
	myAction : function(component, event, helper) {
        ...
        //helper.createCaseInstance(component, event, helper);
	},
    ...
    addProduct : function(component, event, helper){
        ...
        if(caseRec != undefined){
            ...
            if(!productIds.includes(caseRec.Account_Product__c)){
                ...
                //casesToSave.push(newCase);
                component.set("v.CasesToSave", casesToSave);
                console.log('caseRec:'+JSON.stringify(component.get("v.CaseRec")));
            }
            var serviceType = '';
            if(component.get("v.CasesToSave")[0].Service_Type__c == 'Advance Exchange + Onsite'){
                serviceType = 'Advance Exchange + Onsite';
            }
            console.log('serviceType:'+serviceType);
            console.log('caseRec.Account_Product__c:'+caseRec.Account_Product__c);
            var newCase = {"AccountId":caseRec.AccountId,"Customer_Notification_Email__c":caseRec.Customer_Notification_Email__c,"Account_Product__c":'',"ProductId":'',"Out_Of_Scope__c":false,"Asset_Number__c":'',"Service_Type__c":serviceType,"Preferred_Time__c":'',"POS_Register_Number__c":'',"Incident__c":caseRec.Incident__c,"Priority":caseRec.Priority,"Support_Team__c":caseRec.Support_Team__c,"Tech_Notes__c":caseRec.Tech_Notes__c,"Description":caseRec.Description,"Problem_Code__c":''};
            component.set("v.CaseRec", newCase);
        }
        var viewIndex = component.get("v.viewIndex");
        viewIndex++;
        component.set("v.viewIndex",viewIndex);
        console.log('viewIndex:'+viewIndex);
        console.log('CasesToSave length:'+component.get("v.CasesToSave").length);
    },
    
    previousCase : function(component, event, helper){
        var casesToSave = component.get("v.CasesToSave");
        var caseRec = component.get("v.CaseRec");
        console.log('caseRec:'+JSON.stringify(component.get("v.CaseRec")));
        if(caseRec != undefined){
        ...
        console.log('viewIndex:'+viewIndex);
        var casesToSave = component.get("v.CasesToSave");
        component.set("v.CaseRec", casesToSave[viewIndex]);
    },
    
    nextCase : function(component, event, helper){
        var viewIndex = component.get("v.viewIndex");
        viewIndex++;
        component.set("v.viewIndex",viewIndex);
        console.log('viewIndex:'+viewIndex);
    ...
    addAccInCases : function(component, event, helper){
        ...
        //}
        console.log('caseRec for Acc:'+JSON.stringify(caseRec));
        component.set("v.CaseRec",caseRec);
    },
    ...
    saveRecords : function(component, event, helper){
        ...
        if(clList.length != 0){
            for(var i = 0; i < clList.length; i++){
                ...
                if(acId == null || acId == '' || acId == undefined || clList[i].ProductId == '' || clList[i].ProductId == null || clList[i].ProductId == undefined || clList[i].Service_Type__c == '' || clList[i].Service_Type__c == null || clList[i].Service_Type__c == undefined){
                    ...
                    toastEvent.setParams({
                        title : 'Error',
                        message : 'Please fill in location of the service, Product and Service Type!',
                        duration : ' 5000',
                        key : 'info_alt',
                        type : 'error',
                        mode : 'sticky'
